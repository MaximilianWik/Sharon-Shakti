...
- **Home statement backdrop** — `DecorBackdrop` at `opacity 0.6` behind "The skin remembers what the eye fears…"; statement text + facets wrapped in `relative z-10`
- **`/care` atmosphere** — `DecorBackdrop` layered on both the header (`opacity 0.44`, `objectPosition center 30%`) and aftercare sections (`opacity 0.4`, `objectPosition center 70%`); content stays above. `EmberField` removed from both sections
